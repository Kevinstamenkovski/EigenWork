...
`MotorPositionController` applies the protected PID at 10 ms to the 5 ms `MotorAssembly` plant. Its Kp/Ki/Kd/target values live in a `ValidatedParameterSet` whose immutable schemas define finite ranges, defaults, units, and adjustment steps. Accepted changes reconstruct/reset controller state to avoid mixing integral/derivative history across gain changes. The Motor Rig switches between external centered PWM and internal position control. Its setpoint, measurement, error, and controller output are published every 10 ms as bounded 8-bit scope channels. Demo B instead keeps the rig in external mode: motor position feeds MCU GPIO, actual Eigen-8 instructions calculate the control error, and the MCU `pwm_duty` register drives the motor command. Demo C uses the reusable PID. Both paths remain server authoritative and are covered by Minecraft GameTests.
...
`McuCommunicationController` integrates these pure engines behind Eigen-8 ports `0x40`–`0x61`; the CPU initiates work and polls status rather than receiving instantaneous values. The initial UART is a local loopback, I2C targets a register peripheral at `0x48`, and SPI CS0 targets a deterministic inversion peripheral. `CommunicationHubBlockEntity` is a 1 ms loaded server-scheduled adapter exposing all three protocols through block interaction and persistent diagnostics. CAN already uses explicit registered physical topology; future UART/I2C/SPI cabling will follow the same no-scan boundary.

## Robotics architecture

`RobotGraph` owns ordered immutable joint/link topology and caches cumulative homogeneous transforms against joint positions. Revolute and prismatic local transforms compose through the Milestone 9 matrix implementation. `PlanarArmKinematics` provides analytical 2R forward/inverse kinematics, the translational Jacobian, manipulability, and bounded damped-least-squares numerical IK using pivoted solves near singularities. `SerialManipulatorKinematics` accepts one through six standard revolute DH links and computes full 4x4 forward transforms and a 6xN geometric Jacobian.

`LinearTrajectory` and `TrapezoidalTrajectory` provide point-to-point joint commands; the latter automatically selects triangular motion when distance is too short to reach cruise velocity. `PlanarRobotArm` composes topology, IK, and synchronized profiles. `RobotArmBlockEntity` is the 10 ms authoritative adapter with persistent joint/target state and explicit target/diagnostic ports. `RobotWorldNetwork` separately registers only loaded physical joint modules, rebuilds face-adjacent components on load/unload, validates unique contiguous axis indices and physical chain order, and never scans the world per tick. Each module persists axis/angle/link data and sends only authoritative render state; its client renderer rotates a local steel link arm around the joint. World blocks remain fixed anchors rather than moving collision geometry.
...
`CanBus` owns explicitly attached nodes and a bounded standard-frame model. Ready transmissions arbitrate by the dominant lower 11-bit identifier; losers stay queued, the winner occupies the bus for a bitrate-derived frame duration, and completion broadcasts a copied immutable frame. `ImpairedLink` is a deterministic seeded message scheduler layered separately from protocol semantics, with latency/jitter, serialization bandwidth, probabilistic loss, deadlines, and separate drop/timeout accounting.
...
`CanWorldNetwork` is owned once per logical server beside the scheduler and digital network. Loaded `WorldCanElement` block entities register stable dimension/position addresses through Fabric load/unload events. Registration marks topology dirty. A bounded breadth-first graph rebuild groups face-adjacent elements only when dirty; node-to-node adjacency intentionally requires at least one cable endpoint. The resulting element-to-component map is immutable until the next topology change.

Each cached component owns one 500 kbit/s `CanBus` and a mapping between world nodes and pure CAN nodes. The central server tick advances every component in bounded 1 ms quanta, drains receive queues into server-owned block entities, and never asks clients for simulation state. Unloading a cable or node removes it from the registry and invalidates the cache. `CanNodeBlockEntity` persists configuration and traffic diagnostics, while cable state is purely derived from loaded physical topology.

## Build and release architecture

The Gradle wrapper is the only supported build entry point and selects Java 25 through the toolchain resolver. `build` compiles production/GameTest code, runs the Minecraft GameTest server, runs pure JUnit tests, and produces remapped installable and sources JARs. `scripts/verify-release.sh` additionally checks the embedded Fabric version/platform constraints and representative code/assets/data before emitting the installable JAR SHA-256.

GitHub CI runs this boundary for every main-branch push and pull request with Java 25. Tags are intentionally stricter: a tag must equal `v` plus the Gradle project version before the verified installable JAR can be published. Release automation never changes Minecraft/Fabric versions and never publishes the sources archive as the playable artifact.

The optional `resourcepacks/EigenWorks-Industrial-0.2.1/` tree is deliberately separate from the functional mod assets, so presentation can be enabled or replaced without changing simulation behavior. A checked-in high-resolution 5x4 source atlas is deterministically sliced by the pure Java/AWT `ResourcePackTextureBuilder` into 17 block textures, two transparent item textures, and a pack icon. Resource-pack models override the same `eigenworks` model identifiers already selected by the mod's item descriptors; CAN cable and robot-joint overrides preserve their non-cube geometry. `scripts/build-resource-pack.sh` produces the installable ZIP and `scripts/verify-resource-pack.sh` checks format, asset counts, and archive integrity.
